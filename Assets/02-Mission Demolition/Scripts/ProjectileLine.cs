...
public class ProjectileLine : MonoBehaviour
{
    ...
    public Vector3 lastPoint
    {
        get
        {
            if (points == null)
            {
                // If therea re no points, returns Vector3.zero
                return (Vector3.zero);
            }
    ...
    void FixedUpdate()
    {
        if (poi == null)
        {
            ...
            if (FollowCam.POI != null)
            {
                ...
                {
                    return; // Return if it doesn't find poi
                }
            }
            else
            {
        ...
        // If there a poi, it's loc is added every FixedUpdate
        AddPoint();
        if (FollowCam.POI == null)
        {
            // Once Follow Cam.POI is null, make the local poi null too
            poi = null; 
        }
    }
}
